...
import { separateCombClientToId } from '../helpers/common.helper';
import { IClTrxnBody, invCategory, IVTrxn } from '../helpers/commonTypes';
import {
  IAddReissueBody,
  IAirTicketBody,
  IReissueTicketItem,
...
export const airTicketPayloadFormatter = async (
  ...
  for (const item of tickets) {
    ...
    const taxesCommission = item.taxes_commission?.reduce(
      (acc: any, curr: any) => acc + Number(curr.airline_commission),
      0
    );
...
export const airTicketNonCommissionPayloadFormatter = (body: any) => {
  const {
    invoice_agent_commission,
    invoice_combclient_id,
    invoice_sales_date,
  ...
  const ticketsFormat = tickets.map((item: any) => {
    const {
      combined_id: airticket_vendor_combine_id,
      vendor_id: airticket_vendor_id,
    } = separateCombClientToId(item.airticket_comvendor);
    ...
    invoice_discount += numRound(item?.airticket_discount_total);
    invoice_service_charge += numRound(item?.airticket_extra_fee);
    invoice_vat += numRound(item?.airticket_vat);
    invoice_net_total += numRound(item?.airticket_client_price);
    invoice_purchase_total += numRound(item.airticket_purchase_price);
    ...
    const airTicketDetails = {
      ...
      airticket_profit:
        numRound(Number(item.airticket_client_price)) -
        numRound(Number(item.airticket_purchase_price)),
      airticket_sales_date: invoice_sales_date,
    ...
    const VTransPayload: IVTrxn = {
      ...
      vtrxn_particular_id: transactionTypes['INVOICE-AIR TICKET NON COM'],
      vtrxn_type: airTicketDetails?.airticket_vendor_combine_id
        ? 'CREDIT'
        : 'DEBIT',
      vtrxn_pnr: airTicketDetails?.airticket_pnr,
      vtrxn_route: airTicketDetails?.airticket_route_or_sector,
      vtrxn_pax: item?.vendor_name,
      vtrxn_airticket_no: airTicketDetails?.airticket_ticket_no,
    };
  ...
  const invoicePayload = {
    invoice_category_id: invCategory['Air Ticket-Non Commission'],
    invoice_client_id: client_id,
    invoice_combined_id: combined_id,
    invoice_sub_total,
    invoice_discount,
...
export const invoiceReissueFormatter = (body: IAddReissueBody) => {
  const {
    invoice_agent_commission,
    invoice_combclient_id,
    invoice_sales_date,
  ...
  const ticketsFormat = tickets.map((item: any) => {
    ...
    const tkt_commission =
      item.airticket_fare_difference *
    ...
    const airTicketDetails: IReissueTicketItem = {
      ...
      airticket_extra_fee: item.airticket_extra_fee,
      airticket_client_price: item.airticket_client_price,
      airticket_purchase_price: numRound(item.airticket_purchase_price),
      airticket_profit: numRound(item.airticket_profit),
      airticket_pax_name: item.passport_name,
      airticket_sales_date: invoice_sales_date,
      airticket_journey_date: item.airticket_journey_date,
      airticket_return_date: item.airticket_return_date,
  ...
  const invoicePayload = {
    invoice_category_id: invCategory['Air Ticket-Reissue'],
    invoice_client_id: client_id,
    invoice_combined_id: combined_id,
    invoice_sub_total,
